ig.module(
  ...
  EntityEnemySpawner = EntityBounds.extend({
    ...
    spawnTank: function(self) {
      ...
      var bounds = self.getBounds();
      var settings = [{accel: { x:100 }, direction: bounds.visibleLeft, flip: true}, {accel:{x:-100}, direction: bounds.visibleRight, flip: false}];
      var picker = settings[self.getRandomInt(0,1)];

      ig.game.spawnEntity(EntityTank, bounds.visibleRight, 700, picker);
    ...
    spawnSolider: function(self) {
      ...
      ig.game.spawnEntity(EntitySoldier, self.getBounds().visibleRight, 600, settings);
    }
  });
});
